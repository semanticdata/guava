window.onload = function () {
  ...
  function open(f, opts) {
    ...
    // if (f === ".") f = "monotome.md"; // TODO: needs verification, working without?
    read(f, function (body) {
      ...
      if (index.backlinks && index.backlinks[f]) {
        ...
        document.querySelector(".backlinks").innerHTML = `${backlinks.length} ${
        ...
        backlinks.forEach((b) => {
          ...
          if (!dedupedBacklinks[key]) {
            dedupedBacklinks[key] = [];
          }
    ...
    if (entry) {
      if (opts.scrollTo)
        entry.scrollIntoView({ behaviour: "smooth", block: "center" });
  ...
  function indexInject(path, title) {
    ...
    subjectNode.id = path + linkPath.replace(".md", "");
