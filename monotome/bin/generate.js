...
walk(cwd).then((data) => {
  ...
  json("index.json").then((index) => {
    ...
    let files = data
      .map((p) => {
        if (p.substring(process.cwd().length).match(ignore)) {
          return;
        } // if found folder matches ignore rule; skip it
    ...
    Promise.all(
      files.map((f) => {
        return findBacklinks(`${cwd}${f}`, f.slice(1));
      }),
    )
      .then((r) => {
        ...
        r.forEach((item) => {
          item.forEach((backlink) => {
            ...
            if (!index.backlinks[backlink.dst])
              index.backlinks[backlink.dst] = [];
