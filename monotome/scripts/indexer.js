window.addEventListener("DOMContentLoaded", function () {
  ...
  function isModifier(e) {
    return (
      ...
      e.metaKey ||
      e.key === "Shift" ||
      (buffer.length === 0 && e.key === "Backspace")
    );
  }
  ...
  function processSearchQueue(direction) {
    if (!searchQueue || searchQueue.length < 1) {
      return;
    }
    if (direction === "right") {
  ...
  function searchIndex(term) {
    ...
    searchQueue = Object.keys(index.subjects)
      .filter(
        (subject) =>
    ...
    if (searchQueue.length > 0 && !previousMatch) {
      emit("open-file", { file: searchQueue[0] });
    }
  }
});
